...
| FND-208 | low | The chain consumes what `make assurance-inputs` wrote and cannot itself verify that Make ran the command it printed. Inherent: something must run the producer, and Quoin's contract is that the caller states the result. What is true is that the caller states what the bytes say — and TC-010 now checks the declared argv against `make -n assurance-inputs`, so a declared command that Make does not run is a test failure | `Makefile`, `scripts/assurance_chain.py` | correct-requirement-no-evidence |
| FND-209 | medium | The attestation vocabulary is coarser than the producers'. `ROW_RESULTS` transcribes `vacuous` to `not_computed` and `malformed` to `failed`, because Quoin's attestation `result` has four values and the domain documents have six. The finer state is not lost — the producer's own document is the artefact intake retains, so a `vacuous` row survives verbatim in the retained bytes — but the *summary* field is lossy and a reader of the receipt alone would not see the distinction. Stated rather than presented as a one-to-one mapping | `scripts/assurance_chain.py` | wrong-requirement |
