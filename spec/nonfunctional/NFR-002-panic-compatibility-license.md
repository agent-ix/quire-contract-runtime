...
| NFR-002-AC-4 | The evidence collector records every invoked gate's transcripts and numeric exit status, fails closed on command or checksum-fixed-point failure, and is tested for status propagation; the builder verifies its vendored PGM-01 schema and raw Git commit identities, requires command-specific positive corroboration before deriving a passed outcome, preserves failed or inconclusive outcomes, and is tested for identity, digest, role, extension, outcome truthfulness, nonempty work evidence, and validation behavior. | Inspection (TC-007) |
