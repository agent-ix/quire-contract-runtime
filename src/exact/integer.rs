...
/// only when it does not (IR-286): the representation is canonical (`big` is
/// `Some` exactly when the value does not fit, and `small` is then zero), so
...
impl Integer {
    ...
    /// (IR-286: CBMC cannot prove that test's promoting branch unreachable).
    ...
    // the small path never reaches `BigInt` arithmetic (IR-286: CBMC cannot
    // bound `BigInt`'s digit loops on the overflow branch it cannot prove
    // unreachable).
...
/// `BigInt::from(value)` over four fixed 32-bit digits (IR-286):
