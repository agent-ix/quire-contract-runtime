...
Disposition pass 2026-09-26 at `ab7a9302fcf1b5bd88a6aee597a8ba87a9117c47`. There are 9 fix commits
on top of `9f0071a`. Each outcome below was checked against the code and the fix commit, not taken
from the ticket. Both surviving mutants from the review pass were re-run at the new head. The sign
drop in `big_from_i128` is now caught by
`tc_023_generated_integer_and_ordering_against_an_i128_oracle`. The `abs(i64::MIN)` mutant is now
caught by `exact::integer::tests::tc_023_abs_of_i64_min_promotes_to_its_positive_magnitude`.

| FND | Outcome | sha/reason |
|-----|---------|------------|
| FND-001 | fixed | 3c2c95d: tc_023's expected value is built by `oracle()` from the decimal string through `FromStr`/`from_big`, and the sign-drop mutant now fails the test |
| FND-002 | fixed | 5a4aefc, rewritten in ab7a930. Compile-time `ConstAssert` guards: `size_of::<Value>() == 24` and `size_of::<ValueType>() == 40` pin the tag width and every inline payload, plus a per-type `<=` check. They also compile for wasm32 and thumbv7em with `exact` |
| FND-003 | fixed | cff90cf: `tc_023_abs_of_i64_min_promotes_to_its_positive_magnitude`, and the mutant now fails it. Its `.unwrap()` is part of FND-009 |
| FND-004 | fixed | bf652be: `KANI_STACK` wraps `kani`, `kani-mutations` and both Kani producers in `assurance-inputs`. Where the hard limit is finite it falls back to that limit instead of failing |
| FND-005 | fixed | bbaf242: `tests/exact_debug_parity.rs` pins the full rendering of all nine hand-written `Debug` impls. Its naming and trace are FND-011 |
| FND-006 | fixed | d508959: `Integer::big` now checks its precondition with `debug_assert!`. That macro fails `make audit-panic` (FND-009), so this fix has to be redone in a form the audit accepts |
| FND-007 | fixed | 9fdd3bf: a single `small_magnitude_bits` helper |
| FND-008 | fixed | 5d3ea26: the `verification/kani.rs` comment now describes the inline `i64` form and names what the census still excludes |

### New findings in the fix round

| ID | Severity | Summary | Refs |
|----|----------|---------|------|
| FND-009 | high | `make audit-panic` exits 2 at `ab7a930`, and it passed at `9f0071a`. The runtime scan's `assert!` pattern matches `debug_assert!(` at `src/exact/integer.rs:60` (d508959) and the doc-comment text `` `assert!` and `panic!` `` at `src/exact/composite.rs:41` (5a4aefc/ab7a930). Once those two pass, the test-module scan would hit `.unwrap()` at `src/exact_integer_tests.rs:19` (cff90cf). The coder's fix-round log does not run `audit-panic`. Failure scenario: `make ci` goes red at a gate that was green before this round. `debug_assert!` is also a panic path in debug builds, which FR-006-AC-5 excludes from exact sources | src/exact/integer.rs:60, src/exact/composite.rs:41, src/exact_integer_tests.rs:19, scripts/check_panic_surface.sh:4-5 |
| FND-010 | low | `tc_016_exact_sources_have_no_host_float_std_panic_or_unsafe_path` does not catch `debug_assert!`. Its word-boundary rule treats the `_` before `assert!` as part of a longer identifier and skips the hit. That is why `cargo test` stays green while `make audit-panic` fails on the same line. Failure scenario: a panicking `debug_assert!` in an exact source passes FR-006-AC-5's own traced test | tests/exact_outcomes.rs:382-393 |
| FND-011 | low | The five new tests are named `tc_debug_parity_*` with no `tc_NNN` id, unlike every other traced test in the repo. They also carry `Trace: FR-007-AC-6`, while their own module doc says they are not AC-6's oracle. Failure scenario: coverage counts FR-007-AC-6 as backed by a regression pin while its real evidence, the conformance crate, does not compile | tests/exact_debug_parity.rs:33-35, tests/exact_debug_parity.rs:115-117 |

### Gates at `ab7a930` (reviewer-run, `-j 4`; Kani one harness at a time under `systemd-run --user --scope -p MemoryMax=16G`)

| Gate | Result |
|---|---|
| `make lint` | exit 0 |
| `make msrv` | exit 0 |
| `make audit-panic` | **exit 2** (FND-009) |
| Kani ×8 | all exit 0, at or above their floors: 140, 136, 264, 59, 2613−112 unreachable = 2501, 43, 52, 24 |
| `make assurance-inputs` | exit 0 |
| `cargo test --all-features` | exit 101. Only `shared_assurance` tc_009 and `tc_010_the_chain_never_executes_a_producer_and_the_probe_can_prove_it` fail, the same pre-existing failures as on `origin/main` |

**Verdict after the fix round: CHANGES REQUESTED.** FND-001 through FND-008 are fixed. FND-009 is a
new high finding: the fix round turned `make audit-panic` red.

### Dispositions, round 2

Disposition pass 2026-09-26 at `272af361cd9f69076bf69b9c8db2b2ed91160a5f`. There are 4 fix commits on
top of `ab7a930`. Every outcome was checked against the code, a re-run gate or a re-run mutant, never
taken from the ticket.

| FND | Outcome | sha/reason |
|-----|---------|------------|
| FND-006 | fixed | 5f9310f supersedes d508959. `Integer::big` is deleted and the `debug_assert!` with it. `from_big` is now the only place `big` becomes `Some`, and it runs `i64::try_from` first, so the canonical form is enforced by construction and there is no unchecked constructor to call |
| FND-009 | fixed | 5f9310f. `make audit-panic` exits 0 at `272af36`. The composite.rs doc comment no longer names the macros, and `src/exact_integer_tests.rs` builds `2^63` through `From<u64>` instead of `.unwrap()` |
| FND-010 | fixed | 112d3d1. tc_016's forbidden list gains `debug_assert!`, `debug_assert_eq!` and `debug_assert_ne!`. The reviewer inserted each of the three into `src/exact/integer.rs` in turn, and each one failed `tc_016_exact_sources_have_no_host_float_std_panic_or_unsafe_path` |
| FND-011 | fixed | 8780c31. The tests are renamed `tc_035_debug_parity_*` and trace `TC-035, FR-007-AC-13`, both new artifacts. `quire coverage` binds all five to TC-035 and mints an FR-007-AC-13 obligation |

Deleting `big()` does not weaken Kani. All 8 harnesses pass, and 7 of them discharge exactly the
same obligation counts as at `ab7a930`. `tc_003_exact_ieee_numeric_equal_matches_nan_unordered`
goes from 2501 to 2495 discharged (2607 − 112 unreachable), still above its floor of 2417. The 6
fewer checks fit the removal of the `debug_assert!`, which Kani compiled as assertion checks.

### New findings, round 2

| ID | Severity | Summary | Refs |
|----|----------|---------|------|
| FND-012 | low | Stale doc comment. `src/exact_integer_tests.rs`'s module doc still says the private big-form constructor is `pub(crate)`. 5f9310f deleted that constructor, and it was never `pub(crate)`. Failure scenario: a reader looks for an in-crate constructor that bypasses `from_big` and finds none | src/exact_integer_tests.rs:2-4 |
| FND-013 | low | `make msrv` (Rust 1.75) prints two `dead_code` warnings: `struct ConstAssert is never constructed` and `associated constant HOLDS is never used`. They have been there since `ab7a930`, and the round-1 disposition pass missed them. Stable clippy is clean, and the MSRV gate does not deny warnings, so no gate fails. Failure scenario: turning on `-D warnings` for the MSRV check, which NFR-001 describes as checking every declared target at the MSRV, goes red on code the lint gate accepts | src/exact/composite.rs:45-49 |

### Gates at `272af36` (reviewer-run, `-j 4`; Kani one harness at a time under `systemd-run --user --scope -p MemoryMax=16G`)

| Gate | Result |
|---|---|
| `make audit-panic` | exit 0 |
| `make lint` | exit 0 |
| `make msrv` | exit 0, with 2 `dead_code` warnings (FND-013) |
| Kani ×8 | all exit 0: 140, 136, 264, 59, 2607−112 unreachable = 2495, 43, 52, 24, each at or above its floor |
| `make assurance-inputs` | exit 0 |
| `cargo test --all-features` | exit 101. Only `shared_assurance` tc_009 and `tc_010_the_chain_never_executes_a_producer_and_the_probe_can_prove_it` fail, as on `origin/main` |
