...
const EVALUATED: [&str; 12] = [
    "signed-table", "DIV-01", "DIV-02", "DIV-04", "DIV-05", "DIV-06", "DIV-07", "DIV-08", "DIV-10",
    "DIV-11", "DIV-12", "DIV-13",
];
...
fn tc_018_every_tc192_vector_is_evaluated_or_admission_only() {
    ...
    println!("TC-192 agreement: {} evaluated, {} admission-only", EVALUATED.len(), ADMISSION_ONLY.len());
}
...
fn tc_018_signed_table() {
    let table = [
        (DivisionProfile::Truncating, [(2, 1), (-2, 1), (-2, -1), (2, -1)]),
        (DivisionProfile::Floor, [(2, 1), (-3, -2), (-3, 2), (2, -1)]),
        (DivisionProfile::Euclidean, [(2, 1), (-2, 1), (-3, 2), (3, 2)]),
    ];
    for (p, (profile, cells)) in table.into_iter().enumerate() {
        ...
        for ((a, b), (q, r)) in OPERANDS.into_iter().zip(cells) {
            ...
            assert_eq!((pair.quotient(), pair.remainder()), (&int(q), &int(r)), "{profile:?} ({a},{b})");
            assert_eq!(a, b * q + r);
            assert!(r.abs() < b.abs());
            match profile {
...
fn tc_018_div_04_div_05_div_06_mathematical_and_signed_64_domains() {
    ...
    for p in 0..3_usize {
        ...
        let value = |i: usize| outcomes[i].0.clone().completed().map(|p| (p.quotient().clone(), p.remainder().clone()));
...
fn tc_018_div_07_finite_consumers_require_every_bound() {
    ...
    assert_eq!(dispositions, [RequiresBound, RequiresBound, RequiresBound, Supported, Supported]);
}
...
fn tc_018_div_08_exact_tuple_and_named_denials() {
    ...
    use ChargePoint::*;
    let charges = [IntegerDivisionOperands, IntegerDivisionArithmetic, IntegerDivisionDomainPair, IntegerDivisionResultPair];
    assert_eq!(exact.1, charges);
    assert_eq!(exact.2, [3, 0, 0, 0, 0, 0, 0, 2, 4, 2]);
    assert_eq!(short_work.0, Outcome::Incomplete(work_denied(3, IntegerDivisionResultPair)));
    assert_eq!(
        short_result.0,
        Outcome::Incomplete(incomplete(LimitKind::ResultUnits, 1, 0, int(2), IntegerDivisionResultPair))
    );
...
fn tc_018_div_10_mod_charges_only_integer_modulus_points() {
    ...
    use ChargePoint::*;
    let points = [IntegerModulusOperands, IntegerModulusArithmetic, IntegerModulusDomain, IntegerModulusResultRetain];
...
fn tc_018_div_11_zero_divisors_after_the_operands_charge() {
    ...
    assert_eq!(div[1].0, Outcome::Incomplete(work_denied(0, ChargePoint::IntegerDivisionOperands)));
    assert_eq!(rem[1].0, Outcome::Incomplete(work_denied(0, ChargePoint::IntegerModulusOperands)));
}
...
fn tc_018_div_12_mod_domain_refusal_precedes_retention() {
    ...
    assert_eq!(refused.1, [IntegerModulusOperands, IntegerModulusArithmetic, IntegerModulusDomain]);
    assert_eq!(short.0, Outcome::Incomplete(work_denied(2, IntegerModulusDomain)));
}
...
fn tc_018_div_13_first_short_counter_in_field_order() {
    ...
    assert_eq!(
        outcome,
        Outcome::Incomplete(incomplete(LimitKind::IntegerBits, 2, 0, int(3), ChargePoint::IntegerDivisionOperands))
    );
}
