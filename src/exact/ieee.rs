...
impl Format {
    ...
    fn bias(self) -> i64 {
        (1_i64 << self.exponent_bits.saturating_sub(1)).saturating_sub(1)
    }

    fn emin(self) -> i64 {
        1_i64.saturating_sub(self.bias())
    }
    ...
    fn quantum_min(self) -> i64 {
        self.emin().saturating_sub(self.fraction())
    }

    fn sign_mask(self) -> u64 {
        1_u64 << self.exponent_bits.saturating_add(self.fraction_bits)
    }

    fn all_mask(self) -> u64 {
        self.sign_mask().saturating_sub(1) | self.sign_mask()
    }

    fn exponent_ones(self) -> u64 {
        (1_u64 << self.exponent_bits).saturating_sub(1)
    }
    ...
    fn fraction_mask(self) -> u64 {
        (1_u64 << self.fraction_bits).saturating_sub(1)
    }
    ...
    fn max_finite(self, negative: bool) -> u64 {
        self.sign(negative)
            | self
                .exponent_mask()
                .saturating_sub(1_u64 << self.fraction_bits)
            | self.fraction_mask()
    }
...
impl Finite {
    ...
    fn max_part_bits(self) -> u64 {
        ...
        let odd_bits = u64::from(u64::BITS.saturating_sub(odd.leading_zeros()));
        let exponent = self.exponent.saturating_add(i64::from(zeros));
        if exponent >= 0 {
            odd_bits.saturating_add(exponent.unsigned_abs())
        } else {
            odd_bits.max(exponent.unsigned_abs().saturating_add(1))
        }
    }
...
fn decode(value: IeeeValue) -> Class {
    ...
    if u64::from(biased) == format.exponent_ones() {
        ...
        Class::Finite(Finite {
            ...
            exponent: i64::from(biased)
                .saturating_sub(format.bias())
                .saturating_sub(format.fraction()),
        })
    }
}
...
impl Approximation {
    ...
    fn on_grid(format: Format, log2: i64, scaled: impl FnOnce(i64) -> (BigUint, bool)) -> Self {
        let above = log2
            .saturating_sub(format.fraction())
            .saturating_sub(format.quantum_min());
        let (quantum, index) = if above > 0 {
            (log2.saturating_sub(format.fraction()), above.unsigned_abs())
        } else {
            (format.quantum_min(), 0)
        };
        let (q, exact) = scaled(quantum.saturating_sub(2));
...
fn bit_length(value: u64) -> i64 {
    i64::from(u64::BITS.saturating_sub(value.leading_zeros()))
}
    ...
    fn exact(self, format: Format, rounding: RoundingMode) -> Exact {
        match self {
            ...
            Self::Multiply(a, b) => {
                ...
                real_from(format, negative, x * y, ex.saturating_add(ey))
            }
                ...
                match a {
                    Operand::Finite(dividend) => Exact::Real {
                        ...
                        approximation: approximate(
                            ...
                            dividend.exponent.saturating_sub(divisor.exponent),
                            format,
                        ),
                    },
...
fn sum(format: Format, rounding: RoundingMode, left: Term, right: Term) -> Exact {
    ...
    let align = |term: Term| term.value << term.exponent.saturating_sub(exponent).unsigned_abs();
...
fn approximate(
    ...
    let difference = i128::from(numerator.bits()).saturating_sub(i128::from(denominator.bits()));
    ...
    let log2 = if at_least {
        difference
    } else {
        difference.saturating_sub(1)
    }
    .saturating_add(i128::from(exponent));
    let (least, greatest) = (
        format.quantum_min().saturating_sub(3),
        format.emax().saturating_add(2),
    );
    match i64::try_from(log2) {
        Ok(log2) if log2 > least && log2 < greatest => {
            Approximation::on_grid(format, log2, |target| {
                let scale = exponent.saturating_sub(target);
...
fn tiny(format: Format) -> Approximation {
    Approximation::on_grid(format, format.quantum_min().saturating_sub(3), |_| {
        (BigUint::zero(), false)
    })
}
...
fn huge(format: Format) -> Approximation {
    Approximation::on_grid(format, format.emax().saturating_add(2), |_| {
        (
            BigUint::one() << format.fraction_bits.saturating_add(2),
            false,
        )
    })
}
...
fn integer_square_root(value: &BigUint) -> BigUint {
    ...
    let mut estimate = BigUint::one() << (value.bits() >> 1_u32).saturating_add(1);
    loop {
...
fn square_root(format: Format, operand: Finite) -> Approximation {
    ...
    let log2 = bit_length(operand.significand)
        .saturating_sub(1)
        .saturating_add(operand.exponent)
        >> 1_u32;
    Approximation::on_grid(format, log2, |target| {
        // `root / 2^target = sqrt(radicand × 2^(exponent - 2·target))`.
        let scale = operand.exponent.saturating_sub(target.saturating_mul(2));
...
fn round_at(
    ...
    let remainder = approximation.q.saturating_sub(kept << guard.shift());
    ...
    let up = match direction {
        RoundingMode::Exact | RoundingMode::NearestEven => {
            position == Ordering::Greater || (position == Ordering::Equal && kept & 1 == 1)
        }
    ...
    (if up { kept.saturating_add(1) } else { kept }, inexact)
}
...
fn round(format: Format, exact: Exact, rounding: RoundingMode) -> (u64, IeeeFlags) {
    ...
    let normal_bits = precision.saturating_add(1);
    if bit_length(kept) > normal_bits {
        kept >>= 1_u32;
    ...
    if bit_length(kept) == normal_bits && index.saturating_add(1) >= format.exponent_ones() {
    ...
    let tiny = log2 < format.emin()
        && !(log2.saturating_add(1) == format.emin()
    ...
    let bits = format.sign(negative) | (index << format.fraction_bits).saturating_add(kept);
    (bits, flags)
}
