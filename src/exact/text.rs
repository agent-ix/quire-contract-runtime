...
//! `unicode-normalization` table version; the authority pins the identical
//! `unicode-normalization` release, and the TC-186 shared-corpus agreement test
//! compares every profile against it. No locale, case folding, collation or
...
/// `quire.value.text.unicode-17.0.0/v1` requires Unicode 17.0.0 normalization
/// tables: any other table version makes this array length zero and fails the
/// build.
...
impl fmt::Display for EmptyTextBounds {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "text bounds [{}, {}] are empty",
            self.min, self.max
        )
    }
}
...
impl fmt::Display for InvalidUtf8 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "invalid UTF-8 after {} valid bytes",
            self.valid_up_to
        )
    }
}
...
impl fmt::Display for InvalidTextLiteral {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("invalid text literal")
    }
}
...
impl TextPayload {
    ...
    pub fn from_source_literal(spelling: &str) -> Result<Self, InvalidTextLiteral> {
        let text = decode_json_string(spelling).ok_or(InvalidTextLiteral)?;
        Ok(Self {
            text: text.into(),
...
// Fields behind one `Box`: `Value` carries this type inline, and
...
fn prepare<const N: usize>(
    ...
    let sum = |measure: fn(&str) -> usize| {
        payloads.iter().fold(0_u64, |total, payload| {
            ...
            total.saturating_add(length_amount(measure(payload.as_str())))
        })
    };
    ...
    let retained: Vec<&str> = sequences.iter().map(String::as_str).collect();
    check_length(bound, &retained)?;
    Ok(sequences)
}

/// Decode one complete RFC 8259 string token: the opening quote, then
/// unescaped scalars other than `"`, `\\` and C0 controls, or the escapes
/// `\" \\ \/ \b \f \n \r \t \uXXXX` (a high surrogate only as the first half
/// of a surrogate pair), then the closing quote as the final character.
fn decode_json_string(spelling: &str) -> Option<String> {
    ...
    while let Some(scalar) = scalars.next() {
        match scalar {
            ...
            '\\' => decoded.push(decode_escape(&mut scalars)?),
...
/// Decode the escape after a backslash.
fn decode_escape(scalars: &mut Chars<'_>) -> Option<char> {
    Some(match scalars.next()? {
        '"' => '"',
        '\\' => '\\',
        '/' => '/',
        'b' => '\u{8}',
        ...
        't' => '\t',
        'u' => {
            let unit = hex_unit(scalars)?;
            match unit {
                0xD800..=0xDBFF => {
                    if scalars.next()? != '\\' || scalars.next()? != 'u' {
                        return None;
                    }
                    let low = hex_unit(scalars)?;
                    if !(0xDC00..=0xDFFF).contains(&low) {
                        return None;
                    }
                    let high_bits = unit.checked_sub(0xD800)?.checked_shl(10)?;
                    let low_bits = low.checked_sub(0xDC00)?;
                    char::from_u32(high_bits.checked_add(low_bits)?.checked_add(0x1_0000)?)?
                }
                // `char::from_u32` refuses a lone low surrogate.
...
/// Four hexadecimal digits of either case.
fn hex_unit(scalars: &mut Chars<'_>) -> Option<u32> {
    (0..4).try_fold(0_u32, |unit, _| {
        unit.checked_shl(4)?
            .checked_add(scalars.next()?.to_digit(16)?)
    })
}
