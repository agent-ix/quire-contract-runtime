...
//! so they have no variant here; static `ill_typed` refusals precede evaluation
//! and are returned beside, not inside, an outcome.
...
pub enum Refusal {
    ...
    /// An exact conversion or integer arithmetic result is outside the target
    /// integer domain.
    IntegerOutOfDomain,
    ...
    /// A reduced rational arithmetic result is outside its FR-044
    /// `Rational[..]` result domain.
    RationalOutOfDomain,
}
