...
// Fields behind one `Box` (IR-286), for the reason `Rational` gives: no
...
impl Decimal {
    ...
    pub fn compare(&self, other: &Self) -> Ordering {
        ...
        if left_scale >= right_scale {
            compare_shifted(
                &right.coefficient,
                left_scale.saturating_sub(right_scale),
                &left.coefficient,
            )
            .reverse()
        } else {
            compare_shifted(
                &left.coefficient,
                right_scale.saturating_sub(left_scale),
                &right.coefficient,
            )
        }
    }
...
impl ExactLossValue {
    ...
    fn scaled(value: &Rational, ten_exponent: u64) -> Self {
        ...
        Self {
            numerator,
            cofactor,
            twos: denominator_twos.saturating_add(ten_exponent.saturating_sub(cancelled_twos)),
            fives: denominator_fives.saturating_add(ten_exponent.saturating_sub(cancelled_fives)),
        }
    }
}
...
fn split_factor_five(value: &Integer, limit: u64) -> (Integer, u64) {
    ...
    let mut count = 0_u64;
    // Every tried `(5^width, width)` with `width = 2^j`, the last one included.
    let mut powers = Vec::new();
    let mut power = Integer::from(5_i64);
    let mut width = 1_u64;
    loop {
        if width > limit.saturating_sub(count) {
            break;
        }
        ...
        let next = power.mul(&power);
        powers.push((core::mem::replace(&mut power, next), width));
        width = width.saturating_mul(2);
    }
    powers.push((power, width));
...
impl DecimalLoss {
    ...
    pub fn exact(&self) -> Rational {
        // A loss denominator is a product of positive factors.
        Rational::reduce(self.exact.numerator.clone(), self.exact_denominator())
    }
    ...
    match sign(value).cmp(&sign(bound)) {
        ...
        Ordering::Equal => {
            let magnitude = shifted_digits(value, shift)
                .cmp(&Integer::from(bound.decimal_digits()))
                .then_with(|| {
...
impl<'a> Plan<'a> {
    /// The `decimal.arithmetic` `(integer_bits, decimal_digits)`, which
    /// `decimal.rounding` repeats: derived from each aligned operand's
    /// `sbits(c_i,k_i)` and `sdigits(c_i,k_i)`, never from the intermediate.
    fn arithmetic_sizes(&self) -> (Integer, Integer) {
        let aligned = |(coefficient, shift): Shifted<'_>| {
            (
                shifted_bits(coefficient, shift),
                shifted_digits(coefficient, shift),
            )
        };
        let one = Integer::one();
        match self {
            Self::Combine {
                ...
                let ((a1, g1), (a2, g2)) = (aligned(*left), aligned(*right));
                match combine {
            ...
            Self::Divide {
                ...
                let ((a1, g1), (a2, g2)) = (aligned(*numerator), aligned(*denominator));
                (a1.max(a2), g1.max(g2))
            }
            Self::Unary { operand, .. } => aligned((operand, 0)),
        }
    }
...
fn plan(operation: DecimalOperation<'_>, target_scale: u64) -> Plan<'_> {
    ...
    match operation {
        DecimalOperation::Add(a, b) | DecimalOperation::Subtract(a, b) => {
            ...
            Plan::Combine {
                combine,
                left: (ca, scale.saturating_sub(sa)),
                right: (cb, scale.saturating_sub(sb)),
                scale,
            }
        }
        ...
        DecimalOperation::Divide(a, b) => {
            ...
            let ((ca, sa), (cb, sb)) = (parts(a), parts(b));
            let up = target_scale.saturating_add(sb);
            Plan::Divide {
...
fn evaluate(
    ...
    if let Some((coefficient, shift)) = expanded {
        expansion = expansion
            ...
            .exact_size(LimitKind::DecimalDigits, shifted_digits(coefficient, shift));
    }
    meter.charge(expansion)?;

    // Charged from the aligned operands before the intermediate exists.
    ...
    let placed = match in_target_units(&intermediate, target.max_scale()) {
        ...
        TargetUnits::RoundingStep(units) => {
            ...
            // The arithmetic row's amounts bound the rounded coefficient.
            meter.charge(
    ...
    // The upscale into `T` is charged analytically; `retain` materializes
    // `c × 10^k` only after this charge is granted.
    let upscale = placed.upscale(target);
    meter.charge(
        Charge::new(ChargePoint::DecimalResultRetain)
            .size(LimitKind::ScaleExpansion, upscale)
            .exact_size(
                LimitKind::IntegerBits,
                shifted_bits(&placed.coefficient, upscale),
            )
            .exact_size(
                LimitKind::DecimalDigits,
                shifted_digits(&placed.coefficient, upscale),
            )
...
impl DecimalType {
    /// Decide whether `value` is placed at `T` exactly or by a rounding step,
    /// without materializing any coefficient. Strict `exact` refuses a nonzero
    /// discarded digit.
    ...
    /// loss. The caller has charged its size.
...
/// A decided placement of a reduced value `a/b` at `T`, not yet materialized.
pub(crate) struct Placement<'a> {
    value: &'a Rational,
    /// The least exact scale `k <= T`, or `None` for a rounding step.
...
impl Placement<'_> {
    ...
    pub(crate) fn materialize(self, target: &DecimalType) -> Result<Placed, Refusal> {
        match self.terminating {
            Some(scale) => {
                // `a/b = a × (10^k / b) × 10^-k`, bounded by `a × 10^T`.
                let factor =
                    Integer::power_of_ten(u64::from(scale)).exact_div(self.value.denominator());
                Ok(Placed {
                    coefficient: self.value.numerator().mul(&factor),
                    scale,
                    loss: None,
                })
            }
            None => target.round_at_target(self.value),
        }
    }
}
...
fn terminating_scale(denominator: &Integer) -> Option<u64> {
    ...
    let [twos, fives] = counts;
    (remaining == Integer::one()).then_some(twos.max(fives))
}
...
fn in_target_units(intermediate: &Intermediate, target_scale: u32) -> TargetUnits {
    ...
    let excess = match u32::try_from(scale) {
        ...
        _ => scale.saturating_sub(u64::from(target_scale)),
    };
    if value.is_zero() {
        return TargetUnits::Exact {
    ...
    if excess > value.decimal_digits() {
        ...
        let proxy = Rational::reduce(Integer::from(sign), Integer::from(10_i64));
    ...
    if remainder.is_zero() {
        ...
        TargetUnits::RoundingStep(Rational::reduce(value.clone(), power))
    }
}
...
pub(crate) fn shifted_bits(coefficient: &Integer, shift: u64) -> Integer {
...
/// from the unshifted coefficient.
pub(crate) fn shifted_digits(coefficient: &Integer, shift: u64) -> Integer {
