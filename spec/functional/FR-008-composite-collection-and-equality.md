...
- `Value`'s `Debug` and `Drop` are hand-written and iterative over an explicit worklist, not
  ...
  and free without recursing. The hand-written `Debug` renders byte-identically to what
  `#[derive(Debug)]` produced.
...
| FR-008-AC-9 | `Value`'s `Debug` and `Drop` are hand-written and iterative: formatting or dropping a value nested past a recursive walk's host-stack limit does not overflow the stack, and the hand-written `Debug` output is byte-identical to the former derived rendering. | Test (TC-024, TC-025) |
